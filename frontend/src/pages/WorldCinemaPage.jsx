/* WorldCinemaPage — Discover films from around the world
 * Copyright (c) 2026 VAASSEN GmbH / Ray Vaassen
 */
import { useState, useEffect, useCallback, useRef } from "react";
import { Globe, MapPin, Film, ChevronDown, ChevronUp, TrendingUp, Pin, Loader2, Map, LayoutGrid } from "lucide-react";
...
import { matchColor, matchLabel } from "../utils/world-cinema-utils.js";
    ...
    <>
      <div className="page-header">
        <div className="wc-header-row">
          <h2><Globe size={20} style={{ marginRight: 8 }} />World Cinema</h2>
          <div className="wc-view-toggle">
            <button className={viewMode === "map" ? "active" : ""} onClick={() => setViewMode("map")}
              title="Map view"><Map size={16} /><span>Map</span></button>
            <button className={viewMode === "grid" ? "active" : ""} onClick={() => setViewMode("grid")}
              title="Grid view"><LayoutGrid size={16} /><span>Grid</span></button>
          </div>
        </div>
        <p>Discover films and shows from around the world, ranked by your taste profile.
          Click a country to browse trending content.</p>
      </div>
      <div className="page-body">
        {viewMode === "map" ? (
          <WorldMapView countries={allCountries} pinned={pinned}
            selectedCode={selectedCountry?.code} onSelect={selectCountry} />
        ) : (
          <>
