...
function TrendingPage({ onCardClick }) {
          ...
          <div>
            <div className="admin-card" style={{ marginBottom: 12 }}>
              ...
              {sysSettings.services && Object.entries(sysSettings.services).map(([name, svc]) => (
                <div className="service-row" key={name} style={{ padding: "6px 0", borderBottom: "1px solid var(--border)" }}>
                  <span style={{ fontWeight: 500 }}>{name}</span>
                  <span style={{ fontSize: 12, color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace" }}>{svc.url} &middot; {svc.api_key}</span>
                </div>
              ))}
            </div>
            {sysSettings.llm && (
              <div className="admin-card" style={{ marginBottom: 12 }}>
                <h4><Sparkles size={15} /> LLM Integration</h4>
                <div className="service-row"><span>Enabled</span><span>{sysSettings.llm.enabled ? "Yes" : "No"}</span></div>
                {sysSettings.llm.url && <div className="service-row"><span>Endpoint</span><span className="service-detail">{sysSettings.llm.url}</span></div>}
                {sysSettings.llm.chromadb_url && <div className="service-row"><span>ChromaDB</span><span className="service-detail">{sysSettings.llm.chromadb_url}</span></div>}
                {sysSettings.llm.embedding_model && <div className="service-row"><span>Embedding</span><span className="service-detail">{sysSettings.llm.embedding_model}</span></div>}
              </div>
            )}
            <div className="admin-card">
              <h4><Clock size={15} /> Auth</h4>
              <div className="service-row"><span>JWT Expiry</span><span>{sysSettings.auth?.jwt_expiry_hours}h ({Math.round((sysSettings.auth?.jwt_expiry_hours || 0) / 24)}d)</span></div>
            </div>
          </div>
        )}
      </div>
